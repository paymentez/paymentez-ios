...
class StoreViewController: UIViewController {
    ...
    @IBAction func placeOrder(_ sender: Any) {
        if self.selectedCard != nil {
            ...
            var total = 0.0
            for prices in self.productPrices {
                total += prices
            }
            ...
            MyBackendLib.debitCard(uid: UserModel.uid,
                                   cardToken: (self.selectedCard?.token)!,
                                   sessionId: sessionId,
                                   devReference: uuid,
                                   amount: total,
                                   description: "compra",
                                   callback: { (error, trx) in
                DispatchQueue.main.async {
                    self.activityIndicator.stopAnimating()
                    if let transaction = trx {
                        let alertC = UIAlertController(title: "Success",
                                                       message: "transaction_id:\(String(describing: transaction.transactionId!)), status:\(String(describing: transaction.status!)), authcode:\(String(describing: transaction.authorizationCode)), carrier_code:\(String(describing: transaction.carrierCode))",
                        ...
                        let defaultAction2 = UIAlertAction(title: "VERIFY",
                                                           style: .default,
                                                           handler: { (alertaction) in
                            let alertController = UIAlertController(title: "Verificar ",
                                                                    message: "Especifica el valor: Monto o valor",
                                                                    preferredStyle: .alert)
                            alertController.addTextField(configurationHandler: {(_ textField: UITextField) -> Void in
                                textField.placeholder = "Valor"
                            })

                            let verifyActionAmount = UIAlertAction(title: "Verificar por Monto",
                                                                   style: .default,
                                                                   handler: { (alertaction) in
                                MyBackendLib.verifyTrx(uid: UserModel.uid,
                                                       transactionId: transaction.transactionId!,
                                                       type: 0,
                                                       value: (alertController.textFields?[0].text!)!,
                                                       callback: { (error, verified) in
                                    
                                    if verified {
                                        let alertC = UIAlertController(title: "Vefificada",
                                                                       message: "trx:" + transaction.transactionId!,
                                                                       preferredStyle: UIAlertController.Style.alert)
                                        ...
                                        let alertC = UIAlertController(title: "Error en Verificación",
                            ...
                            let verifyActionAmount2 = UIAlertAction(title: "Verificar por Código",
                                                                    style: .default,
                                                                    handler: { (alertaction) in
                                MyBackendLib.verifyTrx(uid: UserModel.uid,
                                                       transactionId: transaction.transactionId!,
                            ...
                            let verifyClose = UIAlertAction(title: "Cancelar",
                                                            style: .cancel,
                                                            handler: { (alertaction) in /* ?? */ })
...
extension StoreViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 1 {
            let alert = UIAlertController(title: "UID",
                                          message: "Please TYPE UID",
                                          preferredStyle: .alert)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        section == 0 ? productPrices.count : 1
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            cell?.detailTextLabel?.text = String(format: "$%.02f",
                                                 self.productPrices[indexPath.row])
            return cell!
