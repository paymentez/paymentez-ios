...
class ListCardsTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.applyStyleNavBar()
        self.refreshTable()
    }

    func applyStyleNavBar() {
        let navigationBar = self.navigationController?.navigationBar
        let titleAttributes: [NSAttributedString.Key: Any] = [.foregroundColor: UIColor.orange]
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
                    ...
                    MyBackendLib.verifyTrx(uid: UserModel.uid,
                        ...
                        if verified {
                            let alertC = UIAlertController(title: "Vefificada",
                                                           message: "trx:"+(card.transactionId!),
        ...
        let deleteAction = UITableViewRowAction(style: .normal, title: "Delete") { (rowAction, indexPath) in
            let card = self.cardList[(indexPath as NSIndexPath).row]
            MyBackendLib.deleteCard(uid: UserModel.uid,
                                    cardToken: card.token!,
                                    callback: { (deleted) in
                if deleted { self.refreshTable() }
            })
        }
        deleteAction.backgroundColor = .red
    ...
    @IBAction func presentAddVC(_ sender:Any?){
        let alertController = UIAlertController(title: "Selecciona el tipo de vista",
                                                message: nil,
                                                preferredStyle: .actionSheet)

        let alertAction = UIAlertAction(title: "Widget in Custom View",
                                        style: UIAlertAction.Style.default) { _ in
            self.performSegue(withIdentifier: "widgetSegue", sender: self)
        }

        let alertAction2 = UIAlertAction(title: "View Controller (Push)",
                                         style: UIAlertAction.Style.default) { _ in
            self.navigationController?.pushPaymentViewController(delegate: self,
                                                                 uid: UserModel.uid,
                                                                 email: UserModel.email)
        }

        let alertModal = UIAlertAction(title: "View Controller (Modal)",
                                         style: UIAlertAction.Style.default) { (_) in
        ...
        alertController.addAction(alertModal)
...
extension ListCardsTableViewController: PaymentCardAddedDelegate{
    func cardAdded(_ error: PaymentSDKError?, _ cardAdded: PaymentCard?) {
        ...
        if cardAdded != nil{
            ...
            let alertController = UIAlertController(title: "Card Status",
                                                    message: statusMsg,
                                                    preferredStyle: .alert)
