...
class MyBackendLib {
    ...
    static func verifyTrx(uid: String,
                          transactionId: String,
                          type: Int,
                          value: String,
                          callback: @escaping (_ error:PaymentSDKError?, _ verified: Bool) -> Void) {
        ...
        let params = ["uid": uid,
                      ...
                      "type": verifyType,
                      "value":value]
        makeRequestPOST("verify-transaction",
                        parameters: params as NSDictionary) { (error, statusCode, responseData) in
            ...
            if statusCode == 200 {
                ...
                do {
                    ...
                    let errorPa =  PaymentSDKError.createError(statusCode!,
                                                               description: dataR["description"] as! String,
                                                               help: dataR["help"] as? String,
                                                               type: dataR["type"] as? String) //parse error in strutcture
    ...
    static func deleteCard(uid: String,
                           cardToken: String,
                           callback: @escaping (_ deleted: Bool) -> Void) {
        let params = ["token": cardToken, "uid": uid]
        makeRequestPOST("delete-card",
                        parameters: params as NSDictionary) { (error, statusCode, responseData) in
            statusCode == 200 ? callback(true) : callback(false)
        }
    }

    static func debitCard(uid: String,
                          cardToken: String,
                          sessionId: String,
                          devReference: String,
                          amount: Double,
                          description: String,
                          callback: @escaping (_ error: NSError?, _ transaction: PaymentTransaction?) -> Void) {
        let params = ["uid": uid,
                      "token": cardToken,
                      "session_id": sessionId,
                      "amount": String(format: "%.2f", amount),
                      "dev_reference":devReference,
                      "description":description] as NSDictionary
    ...
    static func makeRequestGet(_ urlToRequest: String,
                               parameters: NSDictionary,
                               responseCallback: @escaping (_ error: NSError?,
                                                            _ statusCode: Int?,
        ...
        let task = session.dataTask(with: request){ data, resp, err in
            if err == nil {
                ...
                if json == nil{
                    let error = NSError(domain: "JSON",
                                        code: 400,
                                        userInfo: ["parsing": false])
    ...
    static func makeRequestPOST(_ urlToRequest: String,
                                parameters: NSDictionary,
                                responseCallback: @escaping (_ error: NSError?,
                                                             _ statusCode: Int?,
                                                             _ responseData: Any?) -> Void) {
            ...
            if err == nil {
                ...
                if json == nil{
                    let error = NSError(domain: "JSON",
                                        ...
                                        userInfo: ["parsing" : false])
                    print ("parsing json error")
                    resp != nil ? responseCallback(error, (resp as! HTTPURLResponse).statusCode,nil)
                                : responseCallback(error, 400,nil)
                } else {
                    print(json as Any)
                    responseCallback(err as NSError?,
                                     (resp as! HTTPURLResponse).statusCode, json)
                }
            } else {
                responseCallback(err as NSError?, 400, nil)
            }
    ...
    static func encodeParams(_ parameters:NSDictionary) ->Data? {
        var bodyData = ""
        for (key, value) in parameters {
            let scapedKey = (key as! String).addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!
            let scapedValue = (value as! String).addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!
            bodyData += "\(scapedKey)=\(scapedValue)&"
        }
        print(bodyData)
