    ...
    class MyBackendLib
    {
            ...
            makeRequestPOST("verify-transaction", parameters: params as NSDictionary) { (error, statusCode, responseData) in
                ...
                else
                {
                    do {
                        ...
                        let errorPa =  PaymentezSDKError.createError(statusCode!, description: dataR["description"] as! String, help: dataR["help"] as? String, type: dataR["type"] as? String) //parse error in strutcture
        ...
        static func debitCard(uid:String, cardToken:String, sessionId:String, devReference:String, amount:Double, description:String, callback:@escaping (_ error:NSError?, _ transaction:PaymentezTransaction?) ->Void)
        {
